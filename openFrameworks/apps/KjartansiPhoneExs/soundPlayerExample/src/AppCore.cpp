...
void AppCore::draw() {
	...
	float x = 0, y = ofGetHeight()/4;
	float w = ofGetWidth() / (float) scopeArray.size(), h = ofGetHeight()/4;
...
//--------------------------------------------------------------
void AppCore::sendAccelX(const std::string& message, float value) {
    pd.sendFloat(message, value);
    //cout << "Accel: " << value << endl; 
}
